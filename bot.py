...
import datetime
import re
import exams #file containing a dictionary with every course's exam dates
...
def examDate(bot, update):
    #Sends the exam dates for the specified course
    message = update.message.text.split(" ")
    if (len(message) == 2):
        course = message[1];
        exams.initExamDict()
        if course in exams.examdict:
            bot.sendMessage(chat_id = update.message.chat_id, text = '\n'.join(exams.examdict[course]))
        else:
            bot.sendMessage(chat_id = update.message.chat_id, text = "Desculpe, não encontrei essa matéria")
    else:
            bot.sendMessage(chat_id = update.message.chat_id, text = "Por favor, digite o comando e a matéria que está procurando")
...
examDate_handler = CommandHandler('data', examDate)
...
dispatcher.add_handler(examDate_handler)
